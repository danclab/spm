function [Dnew,meshsourceind]=spm_eeg_simulate(D, prefix, patchmni, simsignal,...
    ormni, woi, noiseLevel, SNRdB, trialind, mnimesh, dipfwhm, nAmdipmom,...
    noiseColor)
% Simulate a number of MSP patches at specified locations on existing mesh
% FORMAT [Dnew,meshsourceind]=spm_eeg_simulate(D, prefix, patchmni, simsignal,...
%                                              ormnni, woi, noiseLevel, SNRdB,...
%                                              trialind, mnimesh, dipfwhm,...
%                                              nAmdipmom, noiseColor);
...
% ormni : dipole orientation (for dipole simulation)
% woi: window of interest in seconds
% noiseLevel: noise level in rms femto Tesla or micro volts
...
% nAmdipmom: dipole moments (in nAm)
% noiseColor: 'white' or 'pink' optional, default = 'white'
%
% Outputs
% Dnew- new dataset
...
if nargin < 13 || isempty(noiseColor)
    noiseColor = 'white';
end

noiseColor = lower(char(noiseColor));
if ~ismember(noiseColor, {'white', 'pink'})
    error('noiseColor must be ''white'' or ''pink''.');
end
...
if ~xor(isempty(noiseLevel),isempty(SNRdB))
    error('Must specify either white noise level or sensor level SNR');
end


[~, b1, c1]=fileparts(D{useind}.fname);
...
fprintf('Furthest distance from dipole location to mesh %3.2f mm\n',max(mnidist));
if max(mnidist)>0.1
...
if ~isempty(SNRdB)
    noiseLevel = meanrmssignal.*(10^(-SNRdB/20));
    fprintf('Setting %s noise to give sensor-level SNR of %d dB\n', noiseColor, SNRdB);
end

Qe=eye(Nchans).*(noiseLevel^2); %% sensor level noise
...
for i=1:Ntrials
    ...
    noise = generate_colored_noise(size(Dnew(:, :, i)), noiseLevel, noiseColor);
...
if isempty(ormni)
    ...
    if ~spm('CmdLine')
        figure;
        ploton=1;
        [~,~,~,~,Csensor]=spm_eeg_assemble_priors(L,Qp,{Qe},ploton);
...
function noise = generate_colored_noise(sz, targetRMS, noiseColor)
...
switch noiseColor
    ...
    case 'white'
        noise = noise .* targetRMS;

    case 'pink'
        nChan = sz(1);
        nTime = sz(2);

        freqs = 0:floor(nTime / 2);
        scale = ones(size(freqs));
        scale(2:end) = 1 ./ sqrt(freqs(2:end));
        scale(1) = 0;
        ...
        for ch = 1:nChan
            ...
            X(pos) = X(pos) .* scale(1:length(pos));
            X(neg) = conj(fliplr(X(2:length(neg) + 1)));
